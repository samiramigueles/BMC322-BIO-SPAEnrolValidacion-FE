...
function ValidateIdentity({ setStep, setErrMsg }) {
    const { selfie, frontDni, backDni, uuid, idxID } = useGlobalContext();
    ...
    const postProcessEnroll = async () => {
        ...
        try {
            const { data } = await axios.post("/api/processEnroll", {
                data: body,
            });
            return data;
        } catch (e) {
    ...
    const postProcessValidation = async () => {
        ...
        try {
            const { data } = await axios.post("api/processValidate", {
                idxID,
                selfie,
                uuid,
            });
    ...
    useEffect(() => {
        ...
        if (processOperation === "E" || processOperation === "W") {
            ...
            postProcessValidation().then((data) => {
                if (data?.Resultado === 1) {
                    handleResponseReceived();
                    window.location.href = "https://www.macro.com.ar/eme2";
        ...
        <Layout>
            <Grid item>
                <Typography variant="h1" color={"primary"}>
                    Estamos registrando tu identidad.
                </Typography>
            </Grid>
            <Grid
                item
    ...
    if (
        sessionStorage.getItem("optionProcess") === "E" ||
        sessionStorage.getItem("optionProcess") === "W"
    ) {
        return renderEnroll();
    } else {
